...
describe('Payment Failed Screen - Complete flow from app launch through all error scenarios', () => {
  ...
  it('should display all error types, test navigation, and user interactions', async () => {
    ...
    await device.openURL({ url: 'microcommit://(payment)/payment-failed?errorType=insufficient_funds&goalName=Save%20for%20Hawaii&commitmentAmount=25&charityName=Red%20Cross' });
    ...
    await device.openURL({ url: 'microcommit://(payment)/payment-failed?errorType=card_declined&goalName=Fitness%20Goal&commitmentAmount=50&charityName=UNICEF' });
    ...
    await device.openURL({ url: 'microcommit://(payment)/payment-failed?errorType=invalid_details&goalName=Home%20Renovation&commitmentAmount=100&charityName=Habitat' });
    ...
    await device.openURL({ url: 'microcommit://(payment)/payment-failed?errorType=expired_card&goalName=Vacation%20Fund&commitmentAmount=75&charityName=WWF' });
    ...
    await device.openURL({ url: 'microcommit://(payment)/payment-failed?errorType=3ds_required&goalName=Emergency%20Fund&commitmentAmount=200&charityName=Doctors%20Without%20Borders' });
    ...
    await device.openURL({ url: 'microcommit://(payment)/payment-failed?errorType=network_error&goalName=Debt%20Payoff&commitmentAmount=150&charityName=Local%20Food%20Bank' });
    ...
    await device.openURL({ url: 'microcommit://(payment)/payment-failed?errorType=unknown&goalName=Investment%20Portfolio&commitmentAmount=300&charityName=Animal%20Shelter' });
    ...
    await device.openURL({ url: 'microcommit://(payment)/payment-failed?errorType=card_declined&goalName=Test%20Goal&commitmentAmount=10&charityName=Test%20Charity' });
    ...
    await device.openURL({ url: 'microcommit://(payment)/payment-failed?errorType=insufficient_funds&goalName=Final%20Test&commitmentAmount=99&charityName=Final%20Charity' });
